...
export async function loadFileSystem({ config, dependencies, eventEmitter, rootKey, username }) {
    ...
    if (!navigator.onLine) {
        ...
        cid = cidLog.newest();
        if (cid)
            manners.log("📓 Working offline, using local CID:", cid.toString());
        ...
        cid = cidLog.newest();
        if (cid)
            manners.log("📓 No DNSLink, using local CID:", cid.toString());
        else
 ...
 */
export async function recoverFileSystem({ auth, dependencies, oldUsername, newUsername, readKey, }) {
    ...
    const proof = await storage.getItem(storage.KEYS.ACCOUNT_UCAN);
...
export async function checkFileSystemVersion(depot, config, filesystemCID) {
    ...
    const versionStr = links[RootBranch.Version] == null
        ? "1.0.0"
        : new TextDecoder().decode(await Protocol.basic.getFile(depot, decodeCID(links[RootBranch.Version].cid)));
 ...
 */
async function getDataRoot(reference, username, options = {}) {
