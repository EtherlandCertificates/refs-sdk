 ...
 */
export const createConsumer = async (dependencies, options) => {
        ...
        switch (ls.step) {
            ...
            case LinkingStep.Delegation:
                if (!ls.sessionKey) {
                    ...
                    const linkingResult = await linkDevice(dependencies.auth, dependencies.crypto, ls.sessionKey, ls.username, message);
                    if (linkingResult.ok) {
 ...
 */
export const generateTemporaryExchangeKey = async (crypto) => {
 ...
 */
export const handleSessionKey = async (crypto, temporaryRsaPrivateKey, data) => {
    ...
    if (parseResult.ok) {
        ...
        try {
            encodedUcan = await crypto.aes.decrypt(Uint8arrays.fromString(msg, "base64pad"), sessionKey, Crypto.SymmAlg.AES_GCM, iv);
        }
        catch {
            return { ok: false, error: new LinkingError("Consumer could not decrypt closed UCAN with provided session key.") };
        }
        const decodedUcan = Ucan.decode(Uint8arrays.toString(encodedUcan, "utf8"));
 ...
 */
export const generateUserChallenge = async (crypto, sessionKey) => {
    const pin = Array.from(crypto.misc.randomNumbers({ amount: 6 })).map(n => n % 9);
    const iv = crypto.misc.randomNumbers({ amount: 16 });
    const msg = await crypto.aes.encrypt(Uint8arrays.fromString(JSON.stringify({
        did: await DID.ucan(crypto),
        pin
    }), "utf8"), sessionKey, Crypto.SymmAlg.AES_GCM, iv);
 ...
 */
export const linkDevice = async (auth, crypto, sessionKey, username, data) => {
    const typeGuard = (message) => {
        return Check.isObject(message)
            && "iv" in message && typeof message.iv === "string"
            && "msg" in message && typeof message.msg === "string";
    ...
    if (parseResult.ok) {
        ...
        const response = tryParseMessage(Uint8arrays.toString(message, "utf8"), Check.isObject, { participant: "Consumer", callSite: "linkDevice" });
        if (!response.ok) {
            return response;
        }
