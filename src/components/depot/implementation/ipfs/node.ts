...
const latestPeerTimeoutIds: {
  [peer: string]: null | ReturnType<typeof setTimeout>
} = {}
...
export async function listPeers(
  ...
  if (t.isString(maybePeers) && maybePeers.trim() !== "") {
    ...
    fetchPeers(peersUrl)
      .then((list) => storage.setItem(storageKey, JSON.stringify(list)))
      .catch((err) => {
        // don't throw
        console.error(err)
      })
...
function keepAlive(
  ...
  if (backoff.currentBackoff < KEEP_TRYING_INTERVAL) {
    ...
    timeoutId = setTimeout(
      () => reconnect(ipfs, peer, backoff, status),
  ...
  ping(ipfs, peer)
    .then(({ latency }) => {
      ...
      if (timeoutId === latestPeerTimeoutIds[peer.toString()]) {
        setTimeout(
          () => keepAlive(ipfs, peer, BACKOFF_INIT, updatedStatus),
          KEEP_ALIVE_INTERVAL
        )
      }
    })
...
export function tryConnecting(
  ...
  ping(ipfs, peer)
    .then(({ latency }) => {
      return ipfs.swarm.connect(peer, { timeout: 60 * 1000 }).then(() => {
...
function report(peer: Multiaddr, status: Status): void {
  ...
  const offline = peerConnections.every(
    (connection) => !connection.status.connected
  )
  const lastConnectedAt: number = peerConnections.reduce(
    (newest, { status }) =>
      newest >= (status.lastConnectedAt || 0)
        ? newest
        : status.lastConnectedAt || 0,
    0
  )

  const activeConnections = peerConnections.filter(
    (connection) => connection.status.latency !== null
  )
  const averageLatency =
    activeConnections.length > 0
      ? peerConnections.reduce(
          (sum, connection) => sum + (connection.status.latency || 0),
          0
  ...
  if (monitoringPeers) {
    ...
    console.log(
      "last connected at",
      lastConnectedAt === 0 ? null : lastConnectedAt
    )
...
export async function monitorBitswap(
  dependencies: Dependencies,
  ipfs: IPFS,
  peersUrl: string,
  verbose: boolean
  ...
  monitor = setInterval(async () => {
    ...
    peerList.map(async (peer) => {
      ...
      const wantList = await ipfs.bitswap.wantlistForPeer(peerId, {
        timeout: 120 * 1000,
      })
            ...
            ;(console.table || console.log)(
              dag.value.Links.map((l: unknown) => {
                if (
                  ...
                  return {
                    name: l.Name,
                    cid: (l.Hash as unknown as string).toString(),
                  }
                } else {
                  return null
                }
