...
export async function lookup(
  ...
  try {
    const cid = await DNS.lookupDnsLink(
      username + ".files." + endpoints.userDomain
    )
...
export async function lookupOnFisson(
  ...
  try {
    const resp = await fetch(Fission.apiUrl(endpoints, `/account/cid`), {
      method: "POST",
      headers: {
...
export async function update(
  ...
  return await fetchWithRetry(
    Fission.apiUrl(endpoints, `user/data/${cid}`),
    {
      headers: async () => {
  ...
  if (retryOptions.retryOn.includes(response.status)) {
    if (retry < retryOptions.retries) {
      return await new Promise((resolve, reject) =>
        setTimeout(
          () =>
            fetchWithRetry(url, retryOptions, fetchOptions, retry + 1).then(
              resolve,
              reject
            ),
